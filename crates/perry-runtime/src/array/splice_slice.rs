...
pub extern "C" fn js_array_splice(
    ...
    unsafe {
        ...
        for i in 0..actual_delete as usize {
            // GC_STORE_AUDIT(INIT): deleted-elements result is rebuilt before being returned.
            ptr::write(
        ...
        if items_count != actual_delete && tail_len > 0 {
            ...
            // GC_STORE_AUDIT(BARRIERED): splice tail shift is followed by array layout rebuild.
        ...
        if items_count > 0 && !items.is_null() {
            for i in 0..items_count as usize {
                // GC_STORE_AUDIT(BARRIERED): splice item write is followed by array layout rebuild.
...
pub extern "C" fn js_array_slice(
    ...
    unsafe {
        ...
        for i in 0..slice_len as usize {
            // GC_STORE_AUDIT(INIT): slice result is unpublished and rebuilt before return.
            ptr::write(
                dst_elements.add(i),
